...
L_New_Entry:
		...
		announce "[Зал Славы] " + getarg(1) + " становится No. " + @startPos + " в списке лучших Зала Славы!", bc_all;
		return;
}

function	script	printHallOfFame	{
	if(getarg(0) == 1) {
		mes "[Зал Славы] - Лучшие прошлой недели";
		for(set @loop, 0; @loop < $HoF_totalCount; set @loop, @loop + 1){
			mes "^ff0000";
			mes "Позиция No. " + (@loop + 1) + ":^0000ff";
			mes "+================================+";
			mes "^000000Имя :" + $HoF_LadderNameO$[@loop];
			mes "Ур. Базы :" + $HoF_LadderBLevelO[@loop];
			mes "Ур. Проф. :" + $HoF_LadderJLevelO[@loop];
			mes "Zeny :" + $HoF_LadderZenyO[@loop] + "^0000ff";
			mes "+================================+^000000";
		}
	} else {
		mes "[Зал Славы] - Лучшие сегодня";
		for(set @loop, 0; @loop < $HoF_totalCount; set @loop, @loop + 1){
			mes "^ff0000";
			mes "Позиция No. " + (@loop + 1) + ":^0000ff";
			mes "+================================+";
			mes "^000000Имя :" + $HoF_LadderName$[@loop];
			mes "Ур. Базы :" + $HoF_LadderBLevel[@loop];
			mes "Ур. Проф. :" + $HoF_LadderJLevel[@loop];
...
prontera,164,134,2	script	Jury	109,{
	mes "[Юрий]";
	mes "Добрый день. Вы хотите увидеть список Зала Славы?";
	next;
	menu "Да",L_OK,"Нет",L_QUIT;

L_OK:
	mes "[Юрий]";
	mes "Вы хотите посмотреть список Лучших сегодня или прошлых дней?";

L_MENU:
	next;
	menu "Текущие", L_CUR, "Прошлые", -,"Никакой",L_QUIT;
